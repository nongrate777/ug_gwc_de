...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (\class_exists('Mihdan\\IndexNow\\Dependencies\\Firebase\\JWT\\JWT')) {
            return \Mihdan\IndexNow\Dependencies\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
        return \Mihdan\IndexNow\Dependencies\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm, $signingKeyId = null)
    {
        if (\class_exists('Mihdan\\IndexNow\\Dependencies\\Firebase\\JWT\\JWT')) {
            return \Mihdan\IndexNow\Dependencies\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
        }
        return \Mihdan\IndexNow\Dependencies\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
    }
    /**
